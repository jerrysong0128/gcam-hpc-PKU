...
PERL_PARSER="${TOOLDIR:-./gcam-hpc-tools}/run-tools/parse-scenario-batch.pl"
SPA_MAP_FILE="${TOOLDIR:-./gcam-hpc-tools}/run-tools/spa-mapping/spa-policy-targets.csv"	 # CSV file mapping SPA codes to policy target files
...
load_spa_map() {
	# Validate the optional SPA mapping file once before permutation generation.
	local csv_file="$1"
	if [[ ! -f "$csv_file" ]]; then
...
lookup_spa_target() {
	# Read one SPA target on demand. This avoids Bash 4 associative arrays and
	# keeps the generator compatible with the Bash 3 shipped by macOS.
	local spa_code="$1"
	awk -F, -v wanted="$spa_code" '
		$1 !~ /^#/ && $1 != "SPAs" {
			gsub(/[[:space:]]/, "", $1)
			gsub(/[[:space:]]/, "", $2)
			if ($1 == wanted) {
...
{	# output redirection
		...
		if [[ $scenario_name =~ (SPA[0-9]{3}) ]]; then
			...
			policy_target_from_map="$(lookup_spa_target "$spa_code")"
		fi
