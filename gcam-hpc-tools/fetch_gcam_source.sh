...
# match the gcam-*/cvs/objects/build/linux pattern that gcam_workspace.setup
# auto-detects.
#
# Usage:
#   ./fetch_gcam_source.sh --variant core  --version gcam-v8.2
#   ./fetch_gcam_source.sh --variant china --version gcam-china-v8
#   ./fetch_gcam_source.sh -v china -t gcam-china-v7.1 --dest /workspace
#
...
usage() {
    cat <<'EOF'
Usage: fetch_gcam_source.sh --variant {core|china} --version TAG [options]
...
# gcam_workspace.setup. We avoid duplicate prefixes when the upstream tag
