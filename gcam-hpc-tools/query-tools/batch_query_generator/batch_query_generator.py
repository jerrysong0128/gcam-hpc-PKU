#!/usr/bin/env python3
"""Build a GCAM batch query XML from query_config.yaml.

Usage:
    python batch_query_generator.py query_config.yaml

The script has no third-party dependencies. It supports the simple YAML format
used by query_config.yaml: top-level keys with string values or lists of strings.
"""
...
def default_output_path() -> Path:
    ...
    return (SCRIPT_DIR.parent / "user_batch_queries" / f"output_queries_{timestamp}.xml").resolve()


def main(argv: List[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build a GCAM batch query XML from query_config.yaml")
    parser.add_argument("config", nargs="?", default=str(SCRIPT_DIR / "query_config.yaml"), help="Path to query_config.yaml")
